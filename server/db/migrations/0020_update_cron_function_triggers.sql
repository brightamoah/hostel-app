...
-- =====================================================
-- 4. TRIGGER: Auto-update billing, allocation & STUDENT on payment
--    (Updated to include Student Activation logic)
-- =====================================================
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD IS NULL OR OLD.status != 'completed') THEN
    ...
    IF FOUND THEN
      ...
      threshold_amount := CAST(billing_record.amount AS NUMERIC) * 0.6;
      ...
      -- 3. Check Activation Threshold (>= 60%)
      IF new_paid_amount >= threshold_amount THEN
        
        -- Fetch allocation to check if it's currently pending
        SELECT * INTO allocation_record FROM allocation WHERE id = billing_record.allocation_id;
        
        IF FOUND AND allocation_record.status = 'pending' THEN
           
           -- A. Activate Allocation
           UPDATE allocation
           SET status = 'active'
           WHERE id = billing_record.allocation_id;
           
           -- B. Activate Student
           -- Explicitly cast to enum type to be safe
           UPDATE student
           SET residency_status = 'active'::residency_status
           WHERE id = allocation_record.student_id;
...
-- Drop and recreate the trigger to ensure it uses the latest function definition
